...
type Msg = PreviewInitMsg | PreviewFrameMsg | PreviewParamsMsg | PreviewResizeMsg;
...
vec2 lens(vec2 uv,float k){ vec2 p=uv*2.-1.; float r=dot(p,p); vec2 pd=p*(1.+k*r); return (pd+1.)*0.5; }
...
function initGL(canvas: OffscreenCanvas, w: number, h: number) {
  ...
  const buf = gl.createBuffer(); const vao_ = gl.createVertexArray(); vao=vao_; gl.bindVertexArray(vao_);
...
self.onmessage = (ev: MessageEvent<Msg>) => {
  const m = ev.data;
  switch (m.kind) {
    case 'init': canvasRef=m.canvas; initGL(canvasRef, m.width, m.height); break;
    case 'params': applyParams(m); break;
    case 'resize':
      if (gl && canvasRef) {
        canvasRef.width=m.width; canvasRef.height=m.height;
        gl.viewport(0,0,m.width,m.height);
        gl.uniform2f(uRes!, m.width, m.height);
      } break;
    case 'frame': draw(m.bitmap, m.ts); break;
  }
};
