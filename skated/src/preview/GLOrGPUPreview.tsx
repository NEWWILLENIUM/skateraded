...
const GPU_URL = new URL('./gpuWorker.ts', import.meta.url);
...
export default function GLOrGPUPreview({ file, params, width=960, height=540, onCanvasReady }: Props) {
  ...
  useEffect(() => {
    ...
    const canGPU = 'gpu' in navigator;
    const worker = new Worker(canGPU ? GPU_URL : GL_URL, { type: 'module' });
  ...
  useEffect(() => {
    workerRef.current?.postMessage({ kind: 'params', ...params });
  }, [params]);
  ...
  useEffect(() => {
    ...
    v.src = url; v.onloadedmetadata = () => v.play().catch(()=>{});
